...
function ForumAdmin({ reports, settings, reviewReport, updateSettings }) {
  return <div className="grid gap-5"><section className="panel overflow-hidden"><div className="border-b border-slate-200 p-5"><h2 className="font-black text-navy">Forum Availability</h2></div><div className="divide-y divide-slate-200">{settings.map((channel) => <div key={channel.channel} className="grid gap-4 p-4 sm:grid-cols-[minmax(180px,1fr)_auto_auto_auto] sm:items-center"><div><p className="font-bold text-navy">#{channel.channel}</p><p className="text-xs text-slate-500">{channel.suspended ? "Posting suspended" : "Open for messages"}</p></div><label className="flex items-center justify-between gap-3 text-xs font-bold text-slate-600"><ImagePlus size={16} /> Pictures <Toggle checked={Boolean(channel.images_enabled)} onChange={(imagesEnabled) => updateSettings(channel.channel, { imagesEnabled })} label={`${channel.channel} pictures`} /></label><label className="flex items-center justify-between gap-3 text-xs font-bold text-slate-600"><Mic size={16} /> Voice <Toggle checked={Boolean(channel.audio_enabled)} onChange={(audioEnabled) => updateSettings(channel.channel, { audioEnabled })} label={`${channel.channel} voice notes`} /></label><label className="flex items-center justify-between gap-3 text-xs font-bold text-slate-600">Open <Toggle checked={!channel.suspended} onChange={(open) => updateSettings(channel.channel, { suspended: !open, suspensionMessage: `#${channel.channel} is temporarily suspended by administration.` })} label={`${channel.channel} open`} /></label></div>)}</div></section><Table headers={["Channel", "Message", "Reporter", "Reason", "Status", "Actions"]}>{reports.map((report) => <tr key={report.id} className="border-t border-slate-200"><td className="p-4 font-bold">#{report.channel}</td><td className="max-w-sm p-4"><p className="font-bold">{report.author}{report.author_real_name && report.author_real_name !== report.author ? ` · ${report.author_real_name}` : ""}</p><p className="truncate text-sm text-slate-500">{report.body || (report.message_type === "audio" ? "Voice note" : "Picture")}</p></td><td className="p-4">{report.reporter_name}</td><td className="p-4">{report.reason}</td><td className="p-4"><StatusBadge status={report.status} /></td><td className="p-4">{report.status === "open" && <div className="flex gap-2"><button onClick={() => reviewReport(report.id, "actioned")} className="btn-primary px-3 py-1.5">Remove</button><button onClick={() => reviewReport(report.id, "dismissed")} className="btn-secondary px-3 py-1.5">Dismiss</button></div>}</td></tr>)}</Table></div>;
}
