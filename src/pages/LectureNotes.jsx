...
export default function LectureNotes() {
  ...
  const [scopeDepartment, setScopeDepartment] = useState(null);
  ...
  async function load(next = filters) {
    setLoading(true);
    try { const params = new URLSearchParams(Object.entries(next).filter(([, value]) => value)); const data = await api(`/notes?${params}`); setNotes(data.notes || []); setScopeDepartment(data.scopeDepartment || null); } finally { setLoading(false); }
  }
  ...
  return <div className="page-shell"><PageHeader eyebrow="Academics" title="Lecture Notes" description="Search published course material and open it securely from the portal." />
    {scopeDepartment && <p className="mb-3 text-sm font-bold text-teal-800">Showing notes for {scopeDepartment}</p>}
    <form onSubmit={search} className="mb-6 grid gap-3 border-y border-slate-200 bg-white p-4 sm:grid-cols-2 lg:grid-cols-6"><label className="lg:col-span-2"><span className="sr-only">Search notes</span><div className="relative"><Search className="absolute left-3 top-2.5 text-slate-400" size={18} /><input className="field pl-10" value={filters.q} onChange={(event) => setFilters({ ...filters, q: event.target.value })} placeholder="Course code, title, or lecturer" /></div></label>{canManage && <Filter label="Department" value={filters.department} onChange={(value) => setFilters({ ...filters, department: value })} options={departments} />}<Filter label="Level" value={filters.level} onChange={(value) => setFilters({ ...filters, level: value })} options={["Level 200", "Level 300", "Level 400"]} /><Filter label="Semester" value={filters.semester} onChange={(value) => setFilters({ ...filters, semester: value })} options={["First Semester", "Second Semester"]} /><button className="btn-primary"><Search size={17} /> Search</button></form>
    <input ref={replaceRef} className="sr-only" type="file" accept=".pdf,.docx" onChange={replace} />
    <div className="overflow-x-auto border-y border-slate-200 bg-white"><table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="p-4">Course</th><th className="p-4">Lecturer</th><th className="p-4">Class</th><th className="p-4">File</th><th className="p-4">Status</th><th className="p-4">Actions</th></tr></thead><tbody>{!loading && notes.map((note) => <tr className="border-t border-slate-200" key={note.id}><td className="p-4"><p className="font-black text-navy">{note.course_code}</p><p className="text-slate-500">{note.course_title}</p></td><td className="p-4">{note.lecturer_name}</td><td className="p-4 text-slate-600">{note.department}<br />{note.level} | {note.semester} | {note.academic_year}</td><td className="p-4"><BookOpen className="mr-2 inline text-teal-700" size={17} />{note.original_name}</td><td className="p-4"><StatusBadge status={note.status || (note.published ? "published" : "unpublished")} /></td><td className="p-4"><div className="flex items-center gap-1"><a className="p-2 text-teal-800" href={note.file_url} target="_blank" rel="noreferrer" aria-label="View note"><Eye size={18} /></a><a className="p-2 text-teal-800" href={note.download_url} aria-label="Download note"><Download size={18} /></a>{canManage && <><button className="p-2 text-slate-600" onClick={() => toggle(note)} aria-label={note.status === "published" ? "Unpublish note" : "Publish note"}><FileUp size={18} /></button><button className="p-2 text-slate-600" onClick={() => { setReplaceTarget(note); replaceRef.current?.click(); }} aria-label="Replace note"><UploadCloud size={18} /></button><button className="p-2 text-rose-700" onClick={() => remove(note)} aria-label="Delete note"><Trash2 size={18} /></button></>}</div></td></tr>)}</tbody></table>{loading && <p className="p-8 text-center text-sm text-slate-500">Loading lecture notes...</p>}{!loading && !notes.length && <p className="p-8 text-center text-sm text-slate-500">No lecture notes match this search.</p>}</div>
  </div>;
}
